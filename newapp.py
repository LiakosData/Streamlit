...
def download_stock_data():
    """Read stock data from MySQL and prepare for analysis"""
    try:
        ...
        # Read from database
        query = "SELECT * FROM stock_prices ORDER BY Date, ticker"
        df = pd.read_sql(query, engine)
        
        # Pivot the data back to wide format (tickers as columns)
        df_pivoted = df.pivot(index='Date', columns='ticker', values='price')
...
# ✅ Process data only for selected stock
@st.cache_data(ttl=24*60*60)
def get_processed_stock_data(df, selected_ticker):
    """Process and cache data for the selected stock"""
    try:
        results = {
            'returns': get_yearly_returns().loc[lambda x: x['Ticker'] == selected_ticker],
            'sharpe': get_sharpe_ratio().loc[lambda x: x['Ticker'] == selected_ticker],
            'rsi': calc_rsi()[selected_ticker],
            'ma': get_MA()[[f'{selected_ticker}_SMA20', f'{selected_ticker}_SMA50']],
            'macd': get_MACD()[[f'{selected_ticker}_MACD', f'{selected_ticker}_Signal', f'{selected_ticker}_Hist']],
            'price': get_price()[selected_ticker]
        }
        return results
    except Exception as e:
        st.error(f"Error processing data for {selected_ticker}: {str(e)}")
        return None
