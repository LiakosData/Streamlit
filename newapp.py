...
def download_stock_data():
    ...
    try:
        ...
        # Download data with retry session
        df = yf.download(Tickers, period='1y', progress=False, session=session)
        
        if df.empty:
            st.error("No data was downloaded. Please check your internet connection.")
            return None
        ...
        if df.isnull().all().all():
            st.error("Downloaded data is all null. Please check the ticker symbols.")
            return None
